...
$desc_three = '/^Farmers Insurance OUTBOUND$/';
...
foreach ( $recs as $rec ){
	if ( (preg_match($desc_one, ($rec->campaign_name)) || preg_match($desc_two, ($rec->campaign_name))) && (!(preg_match($desc_three, ($rec->campaign_name)))) && (!(preg_match($desc_four, ($rec->skill_name)))) && (!(preg_match($desc_five, ($rec->skill_name))))){
		$farm_rec[] = $rec;
	}
}
